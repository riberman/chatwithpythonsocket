...
def sendMsg(connection, flag):
    ...
    if (mensagem == "see ya"):
        ...
        sendMsg(connection, flag)

def receiveData(connection, flag):
    while (flag):
...
threading.Thread(target=sendMsg, args=(connection, flag,)).start()
threading.Thread(target=receiveData, args=(connection, flag,)).start()
# Finalizar a conexao
# connection.close()
